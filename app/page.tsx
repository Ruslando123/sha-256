  ...
  return (
    <svg viewBox="0 0 400 400" className="w-full max-w-[380px]">
      ...
      {RADAR_AXES.map((label, i) => {
        ...
        return (
          <text key={i} x={p.x} y={p.y} textAnchor="middle" dominantBaseline="middle" fill="rgba(255,255,255,0.45)" fontSize="11">
            {label}
          </text>
        );
      })}
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: `0 0 24px ${alg.color}0d`,
      }}
    ...
    >
      ...
      <p className="text-sm italic text-zinc-400 rounded-lg p-3 leading-relaxed" style={{ background: `${alg.color}0f` }}>
        {alg.quote}
      </p>
      ...
      <div className="grid grid-cols-2 gap-2">
        {[
          ...
          <div key={label} className="rounded-lg p-3" style={{ background: "#1e2030" }}>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-1">
          {alg.usages.map((u) => (
            <span key={u} className="text-xs rounded-full px-2 py-0.5 bg-zinc-800 text-zinc-400 border border-zinc-700">{u}</span>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="overflow-x-auto rounded-xl border border-zinc-800">
      <table className="min-w-full text-sm text-left">
        <thead className="border-b border-zinc-800 text-xs uppercase tracking-wide text-zinc-500" style={{ background: "#1a1d2e" }}>
          <tr>
        ...
        <tbody>
          {algorithms.map((alg) => (
            <tr key={alg.id} className="border-b border-zinc-800 transition" style={{ background: "#12141f" }}
            ...
            >
              ...
              <td className="px-4 py-3 font-mono text-zinc-400">{alg.digestBits} bit</td>
              <td className="px-4 py-3"><SecurityBadge status={alg.security} /></td>
              <td className="px-4 py-3 text-zinc-400">~{alg.speedMBps} MB/s</td>
...
function HistoryTab() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="rounded-xl border border-zinc-800 p-6" style={{ background: "#1a1d2e" }}>
        <h2 className="text-xl font-bold text-white mb-1">History of Hash Algorithms</h2>
        ...
        <div className="relative mb-16">
          <div className="absolute left-0 right-0 h-px bg-zinc-700" style={{ top: "28px" }} />
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {algorithms.map((alg) => (
            <div key={alg.id} className="rounded-xl p-4 border-l-2" style={{ background: "#12141f", borderColor: alg.color }}>
              <div className="flex justify-between items-start mb-1">
                <h3 className="font-black" style={{ color: alg.color }}>{alg.name}</h3>
                <span className="text-xs text-zinc-500">{alg.year}</span>
              </div>
              <p className="text-xs text-zinc-400">{alg.author}</p>
...
function TryItTab() {
  ...
  return (
    <div className="grid md:grid-cols-2 gap-6">
      <div className="rounded-xl border border-zinc-800 p-5 flex flex-col gap-4" style={{ background: "#1a1d2e" }}>
        <div>
          <h2 className="text-lg font-bold text-white">✏️ Live Hasher</h2>
          <p className="text-sm text-zinc-400 mt-1">Введите любой текст и посмотрите, как выглядит его хеш. Даже одна буква меняет всё!</p>
        </div>
        <input
          value={input}
          onChange={(e) => setInput(e.target.value)}
          className="w-full rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-cyan-500 transition"
          style={{ background: "#12141f", border: "1px solid #3f3f46" }}
        />
        <div className="flex flex-col gap-2">
          <div className="rounded-lg p-3" style={{ background: "#12141f" }}>
          ...
          <div className="rounded-lg p-3" style={{ background: "#12141f" }}>
            ...
            <p className="text-xs font-mono text-zinc-300 break-all">{hashes.sha1 ?? "..."}</p>
          </div>
          <div className="rounded-lg p-3" style={{ background: "#12141f" }}>
            <p className="text-xs font-semibold mb-1" style={{ color: "#10b981" }}>SHA-256</p>
            <p className="text-xs font-mono text-zinc-300 break-all">{hashes.sha256 ?? "..."}</p>
          </div>
        </div>
      ...
      <div className="rounded-xl border border-zinc-800 p-5" style={{ background: "#1a1d2e" }}>
        <h2 className="text-lg font-bold text-white mb-4">📚 Key Properties</h2>
...
function WhenToUseTab() {
  return (
    <div className="flex flex-col gap-6">
      <div className="rounded-xl border border-zinc-800 p-6" style={{ background: "#1a1d2e" }}>
        <h2 className="text-xl font-bold text-white mb-1">🗂️ When to Use What</h2>
        <p className="text-sm text-zinc-400 mb-6">Algorithm choice depends on your use case. Here&apos;s a practical guide for developers and beginners.</p>
        <div className="grid md:grid-cols-3 gap-4">
          {useCases.map(({ icon, title, desc, good, bad }) => (
            <div key={title} className="rounded-xl p-4 border border-zinc-700" style={{ background: "#12141f" }}>
              <p className="text-2xl mb-2">{icon}</p>
              <h3 className="font-bold text-white mb-1 text-sm">{title}</h3>
              <p className="text-xs text-zinc-400 mb-3 leading-relaxed">{desc}</p>
      ...
      <div className="rounded-xl border border-zinc-800 p-6" style={{ background: "#1a1d2e" }}>
        <h2 className="text-xl font-bold text-white mb-4">⚡ Quick Rule</h2>
...
export default function AlgorithmEvolutionPage() {
  ...
  return (
    <div className="min-h-screen px-4 py-8" style={{ background: "#0d0f1a", color: "#e4e4e7" }}>
      <div className="mx-auto w-full max-w-6xl flex flex-col gap-6">
        ...
        <header>
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-xs font-semibold tracking-widest text-zinc-500 uppercase">Cryptography Learning Course</p>
          </div>
        ...
        {/* Tabs */}
        <div className="flex gap-1 rounded-xl p-1" style={{ background: "#1a1d2e" }}>
          {tabs.map(({ id, label }) => (
            <button
              key={id}
              onClick={() => setTab(id)}
              className={`flex-1 rounded-lg px-4 py-2 text-sm font-medium transition-all ${
                tab === id ? "text-white border border-zinc-600" : "text-zinc-500 hover:text-zinc-300"
              }`}
              style={tab === id ? { background: "#2a2d3e" } : {}}
            >
              {label}
            </button>
          ))}
              ...
              <div className="flex rounded-lg overflow-hidden border border-zinc-700">
                {(["cards", "table"] as const).map((m) => (
                  <button
                    ...
                    style={{
                      background: viewMode === m ? "#2a2d3e" : "#12141f",
                      color: viewMode === m ? "#fff" : "#71717a",
                    }}
                  >
            ...
            <div className="rounded-xl border border-zinc-800 p-6" style={{ background: "#1a1d2e" }}>
              <div className="flex flex-col md:flex-row gap-6 items-center">
                <div className="flex-1">
                  <h2 className="text-xl font-bold text-white mb-2">Comparison Radar</h2>
                  ...
                  <div className="flex flex-wrap gap-3">
                    {algorithms.map((alg) => (
                      <div key={alg.id} className="flex items-center gap-1.5">
                        ...
                        <span className="text-xs text-zinc-400">{alg.name}</span>
                      </div>
                    ))}
                  </div>
                </div>
